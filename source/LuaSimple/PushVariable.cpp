...
void PushVariable::String(std::string string){
    lua_pushlstring(*(this->pointer_to_lua_state), string.c_str(), string.size());
    return;
}

void PushVariable::TableKeyAdder(std::variant<std::string,lua_Integer> key){
...
void PushVariable::Table(lua_Table table_to_add, bool make_new/*if not set will instead merge the keys and values into existing table on top of stack, presubably overriding any duplicate keys with new ones*/, std::optional<std::string> metatable_name){
    if (make_new){
...
std::any* PushVariable::Userdata(std::any data, std::optional<std::string> metatable_name /*,int uservalue_count*/){

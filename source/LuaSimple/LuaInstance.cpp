...
void LuaInstance::PushTableValue(lua_TableValue to_push){
    if (holds_alternative<void*>(to_push))
    {
        lua_pushlightuserdata(this->pointer_to_lua_state, get<void*>(to_push));
    }
    else if (holds_alternative<lua_Integer>(to_push))
...
lua_TableValue LuaInstance::PopTableValue() {
    lua_TableValue popped;
    switch (lua_type(this->pointer_to_lua_state, -1))
    {
    case LUA_TBOOLEAN:
    {

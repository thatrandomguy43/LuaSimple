...
lua_Value LuaInstance::PopValue() {
    ...
    case LUA_TTABLE:
    {
        ...
        while (lua_next(this->pointer_to_lua_state, -1) != 0)
        {
            ...
            key = this->PopValue();
            get<unique_ptr<lua_Table>>(popped)->table_contents.insert(make_pair(key, value));
        }
        if (luaL_getmetafield(this->pointer_to_lua_state, -1, "__name") != LUA_TNIL){
            get<unique_ptr<lua_Table>>(popped)->metatable_name = get<string>(this->PopValue());
        };
    }
    break;
    case LUA_TFUNCTION:
...
void LuaInstance::PushValue(lua_Value to_push){
    ...
    else if (holds_alternative<unique_ptr<lua_Table>>(to_push))
    {
        ...
        for (auto itr = get<unique_ptr<lua_Table>>(to_push)->table_contents.begin(); itr != get<unique_ptr<lua_Table>>(to_push)->table_contents.end(); itr++){
